...
## §1 Recap (user-language prose)

<2-5 sentences in the user's language. State which feature was verified in a real browser on the
deployed environment (<env>), which variants passed or failed, what follow-up is needed, and which
command re-runs the smoke test. A stakeholder should be able to make a release decision from this
paragraph alone.>
...
|---|---|---|---|---|---|---|
| 1 | login-sso | frontend-web | `/login` | SSO click → provider | dashboard heading "class dashboard" visible | `specs/login-sso.spec.ts` |
| 2 | assign-class | frontend-web | `/class/:id` | click Assign student button | roster count +1 | `specs/assign-class.spec.ts` |
| 3 | live-attendance-ws | frontend-web | `/class/:id/live` | peer-join via 2nd context | attendance-count = 2 | `specs/live-attendance.spec.ts` |
| 4 | broadcast-sse | frontend-web | `/dashboard` | API-triggered broadcast | toast "new assignment" visible | `specs/broadcast.spec.ts` |

Endpoints from `verify.md §5` without a UI caller (gap):
- `POST /api/internal/sync` — Feign-only, no UI → [INFO] (not a gap)
- `GET /api/students/{id}/history` — has UI caller in content-web but not in scope this ticket → [GAP]

## §4 Environment & Auth

| field | value |
|---|---|
| BASE_URL | `https://dev-app.example.test` |
| QA_ENV | dev |
| storageState | `e2e/storageState.dev.json` (redacted, refreshed at <HH:MM>) |
| auth path | test-JWT via `/test/generate-jwt?userId=<id>&role=USER` (see verify/jwt-auth-reference.md) |
| prod-write flag | N/A |
| tenant IDs | `fixtures/env-tenants.ts §dev` (classId 1001, teacherId test-t-001) |
| PII redactions | email→user+<id>@example.test, names→student<id>, Authorization→Bearer __REDACTED__ |
...
assign-class         happy        dev   ✓       4180          4/4         1/3       —
assign-class         boundary     dev   ✓       4112          3/3         1/3       100-char class name
assign-class         negative     dev   ✓       1230          2/2         1/3       403 shown correctly
live-attendance-ws   happy        dev   ⚠       7820          4/4         2/3       flake: WS frame arrived 1200ms late on attempt 1
...
assign-class         happy-smoke  dev   ✓       4050          4/4         1/3       smoke
...
| `specs/assign-class.spec.ts` | modified | button label changed in work.md §4 row 2 | L23 (locator text) |
| `specs/live-attendance.spec.ts` | added | new WS flow in work.md §4 row 3 | +78 lines |
| `specs/broadcast.spec.ts` | preserved | contract unchanged | 0 |
| `specs/smoke.spec.ts` | modified | added happy-smoke for live-attendance | +12 lines |
...
| 1 | assign-class/happy | FAIL-UI | →W (work) | locator getByRole('button',{name:'Add student'}) not found; likely-source frontend-web/src/components/ClassDetail.vue:142 | work.md §4 row "2026-05-03 btn label" added; redeploy; re-run passed |
...
|---|---|---|---|
| `artifacts/run-20260421-1430-dev/` | dev | full suite first pass | trace.zip, video.webm (live-attendance only), 14 screenshots, console.log, network.har |
| `artifacts/run-20260421-1445-dev/` | dev | live-attendance re-run post-fix | trace.zip, video.webm, console.log |
...
|---|---|---|---|
| [FLAKE] | live-attendance-ws/happy | WS frame arrives 1200ms late on ~30% of dev runs | Observed under dev-only. Retest on stg. Do not mark pass without 3 clean consecutive dev runs. |
| [GAP] | history-view | `GET /api/students/{id}/history` has UI caller in content-web, not exercised this ticket | Track as future ticket; not blocking. |
...
```
2026-04-21 14:30 dev  pass             — first-run, all 8 variants pass, 1 flake on live-attendance
2026-04-21 16:12 stg  pass             — smoke only, 3 variants pass on stg
2026-05-03 10:05 dev  pass-with-flake  — update-run after work.md button label change, live-attendance flake recurs
...
When the ticket has a `docs/features/<YYYY-MM-DD>-<slug>.md` or `docs/bugs/...md` from `/work`, append a "Deployment QA (/qa-engineer)" section:

```markdown
## Deployment QA (/qa-engineer)

- **Environment**: <env> (`<BASE_URL>`)
- **Verified at**: <YYYY-MM-DD HH:MM local time>
- **Status**: <pass | pass-with-flake | fail-escalated>
- **Scope**: <N flows, M variants, K smoke tests>
- **Evidence**: `.backend/<YYYYMM>/<slug>/qa.md`, trace `.backend/.../e2e/artifacts/run-<ts>-<env>/trace.zip`
- **Re-run**: `cd .backend/<YYYYMM>/<slug>/e2e && QA_ENV=<env> npx playwright test specs/smoke.spec.ts`
- **Open items**: <§9 [BLOCK]/[FLAKE] summary, or "none">
```
